class <%= migration_class_name %> < ActiveRecord::Migration
  def up
    ...
    begin
      new.original_tsquery = to_tsquery(<%=search_config_name %>, coalesce(new.original,''))
      new.substitution_tsquery = to_tsquery(<%=search_config_name %>, coalesce(new.substitution,''))
      return new;
    end
    $$ LANGUAGE plpgsql;
    EOS
