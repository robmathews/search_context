class AddFoosSearchConfig < ActiveRecord::Migration
  def up
     ...
     execute %Q{CREATE TEXT SEARCH CONFIGURATION foos_search_config ( COPY = pg_catalog.french );}
    ...
    #    A standard parser is provided, and custom parsers can be created for specific needs.
     execute %{ALTER TEXT SEARCH CONFIGURATION foos_search_config
            DROP MAPPING FOR email, file, float, host, int, sfloat, uint, url, url_path, version}
    # remove all the accent characters. 
     execute %Q{ALTER TEXT SEARCH CONFIGURATION foos_search_config
             ALTER MAPPING FOR hword, hword_part, word
             WITH unaccent, french_stem;}
  end

  def down
    execute "DROP TEXT SEARCH CONFIGURATION foos_search_config"
  end
end
